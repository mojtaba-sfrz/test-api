...
def test_read_all_notes(test_app, monkeypatch):
    ...
    test_data = [
        {"title" : "something", "description" : "somethingelse", "id" : 1},
        {"title" : "someone", "description" : "someone else", "id" : 2},
    ]

    async def mock_get_all(test_app):
        return test_data
...
def test_update_note(test_app, monkeypatch):
    test_update_data = {"tetle" : "sth", "description" : "sth else", "id" : 1}
    ...
    async def mock_put(id, playload):
        return 1

    monkeypatch.setattr(crud, "put", mock_put)

    response = test_app.put(f"notes/1", content=json.dumps(test_update_data))
